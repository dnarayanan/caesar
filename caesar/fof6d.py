...
vel_LL = (
    1.0
)  # velocity space linking length factor, multiplies local velocity dispersion
...
def loadsnap(snap, t0):
    ...
    ghalo = np.array(
        readsnap(snap, "HaloID", "gas"), dtype=int
    )  # Halo ID of gas; 0=not in halo
    ngastot = len(ghalo)
    gas_select = ghalo > 0
    ngastot = np.uint64(len(ghalo))
    pindex = np.arange(
        ngastot, dtype=np.uint64
    )  # keep an index for the original order of the particles
    # Load in gas info for selecting gas particles
    gnh = (
        readsnap(snap, "rho", "gas", units=1)[gas_select]
        * h
        * h
        * 0.76
        * (1 + redshift) ** 3
        / const.m_p.to("g").value
    )  # number density in phys H atoms/cm^3
    ...
    dense_gas_select = (gnh > 0.13) & (
        (gtemp < 1.0e5) | (gsfr > 0)
    )  # dense, cool, or SF gas only
    ...
    gvel = readsnap(snap, "vel", "gas", units=1, suppress=1)[
        gas_select
    ]  # vel in physical km/s
    ghalo = np.array(readsnap(snap, "HaloID", "gas"), dtype=int)[
        gas_select
    ]  # Halo ID of gas; 0=not in halo
    ...
    svel = readsnap(
        snap, "vel", "star", units=1, suppress=1
    )  # star vels in physical km/s
    ...
    try:
        ...
        bvel = readsnap(
            snap, "vel", "bndry", units=1, suppress=1
        )  # BH vels in physical km/s
        bhalo = np.array(readsnap(snap, "HaloID", "bndry"), dtype=int)  # Halo ID of BHs
        nbhtot = np.uint64(len(bpos))
    except:
        print(
            "fof6d : Creating one fake BH particle at origin (not in a halo) to avoid crash."
        )
        bpos = [[0, 0, 0]]
        bvel = [[0, 0, 0]]
        bhalo = [0]
    ...
    pos = np.vstack(
        (gpos, spos, bpos)
    ).T  # transpose gives pos[0] as list of x positions, pos[1] as y, etc
    vel = np.vstack((gvel, svel, bvel)).T  # same for vel
    haloID = np.concatenate((ghalo, shalo, bhalo))  # compile list of halo IDs
    pindex = np.concatenate(
        (
            pindex[gas_select],
            np.arange(ngastot, ngastot + nstartot + nbhtot, dtype=np.uint64),
        )
    )
    print(
        "fof6d : Loaded %d (of %d) gas + %d stars + %g bh = %d total particles [t=%.2f s]"
        % (len(gpos), ngastot, len(spos), len(bpos), len(pindex), time.time() - t0)
    )
...
def fof6d(
    igrp, groups, poslist, vellist, kerneltab, t0, Lbox, fof_LL, vel_LL=None, nfof=0
):
    ...
    if vel_LL != None:
        ...
        siglist = (
            []
        )  # list of boolean arrays storing whether crit is satisfied for each neighbor pair
        ...
        siglist = (
            []
        )  # list of boolean arrays storing whether vel disp crit is satisfied
        for i in range(nactive):
    ...
    for ipart in range(nactive):
        ...
        galind_ngb = np.where(
            siglist[densest], galind_ngb, -1
        )  # apply velocity criterion here
        if (
            len(galind_ngb[galind_ngb >= 0]) > 0
        ):  # if it already has neighbors (incl iteself) in a galaxy...
            galmin = np.unique(
                galind_ngb[galind_ngb >= 0]
            )  # find sorted, unique indices of neighbors' gals
            galind[nlist[1][densest]] = min(
                galmin
            )  # put all neighbors in lowest-# galaxy
            for i in range(1, len(galmin)):  # link all other galaxies to lowest
                if linked[galmin[i]] == -1:
                    linked[galmin[i]] = min(
                        galmin
                    )  # link all other galaxies to lowest index one
                else:
                    linked[galmin[i]] = min(
                        linked[galmin[i]], min(galmin)
                    )  # connect all other galaxies to lowest index
    ...
    for i in range(galcount - 1, -1, -1):
        if linked[i] != i:
            assert linked[i] < i, "Trouble: mis-ordered linking %d > %d" % (
                i,
                linked[i],
            )
    ...
    pcount, bin_edges = np.histogram(
        galind, bins=galcount
    )  # count particles in each galaxy
    for i in range(
        iend - istart
    ):  # set indices of particles in groups with <mingrp members to -1
        if pcount[galind[i]] < mingrp:
            galind[i] = -1
    if len(galind[galind >= 0]) == 0:
        return 0, galind  # if there are no valid groups left, return
    ...
    for i in range(len(galind_unique)):
        galind_inv[
            galind_unique[i]
        ] = i  # create mapping from original groups to unique set
    for i in range(iend - istart):
        if galind[i] >= 0:
            galind[i] = galind_inv[galind[i]]  # re-assign group indices sequentially
    ...
    progress_bar(
        1.0 * groups[igrp][1] / groups[len(groups) - 1][1],
        barLength=50,
        t=time.time() - t0,
    )
...
def sort_parts(pos, vel, haloID, pindex, istart, iend, idir0, key="pos"):
    if key == "pos":
        ...
        sort_ind = np.argsort(
            haloID[istart:iend]
        )  # sort by halo ID (idir0 is irrelevant)
    idir1 = (idir0 + 1) % 3  # these are the other two directions
    idir2 = (idir0 + 2) % 3
    pos[idir0][istart:iend] = pos[idir0][istart:iend][
        sort_ind
    ]  # keep all arrays likewise sorted
    ...
    pindex[istart:iend] = pindex[istart:iend][
        sort_ind
    ]  # keep particle indices in the same order
...
def fof_sorting(groups, pos, vel, haloID, pindex, fof_LL, Lbox, idir):
    ...
    for igrp in range(
        len(oldgroups)
    ):  # loop over old groups, sort within each, find new groups
        ...
        sort_parts(
            pos, vel, haloID, pindex, istart, iend, idir, "pos"
        )  # sort group igrp in given direction
        # create new groups by looking for breaks of dx[idir]>fof_LL between particle positions
        oldpos = pos[idir][istart]
        groups[grpcount][
            0
        ] = istart  # set start of new group to be same as for old group
        for i in range(istart, iend):
            if periodic(pos[idir][i], oldpos, Lbox) > fof_LL or i == iend - 1:
                groups[grpcount][1] = i  # set end of old group to be i
                groups.append(
                    [i, i]
                )  # add new group, starting at i; second index is a dummy that will be overwritten when the end of the group is found
                posrange = [
                    min(pos[idir][groups[grpcount][0] : i]),
                    max(pos[idir][groups[grpcount][0] : i]),
                ]
                grpcount += 1
            oldpos = pos[idir][i]
    ...
    for igrp in range(len(oldgroups)):
        ...
        iend = oldgroups[igrp][1]
        if iend - istart >= mingrp:
            groups.append(oldgroups[igrp])
    return groups
...
def fofrad(snap, nproc, mingrp, LL_factor, vel_LL):
    ...
    h = readheader(snap, "h")
    Lbox = Lbox / h  # to kpc
    n_side = int(readhead(snap, "dmcount") ** (1.0 / 3.0) + 0.5)
    MIS = Lbox / n_side
    ...
    for i in range(1, len(haloID)):  # set up groups[]
        ...
        if haloID[i] > haloID[i - 1] and haloID[i - 1] > 0:
            if i - hstart >= mingrp:
                groups.append([hstart, i])
            hstart = i

    # within each group (i.e. halo), create sub-groups of particles via directional sorting, ala FOFRAD
    print(
        "fof6d : FOF via sorting beginning with %d halos/groups [t=%.2f s]"
        % (len(groups), time.time() - t0)
    )
    for idir in range(
        len(pos)
    ):  # sort in each direction, find groups within sorted list
        groups = fof_sorting(groups, pos, vel, haloID, pindex, fof_LL, Lbox, idir)
        print(
            "fof6d : Axis %d approx FOF via sorting found %d groups [t=%.2f s]"
            % (idir, len(groups), time.time() - t0)
        )
    ...
    if nproc > 1:
        npart_par = (
            2000
        )  # optimization: groups with > this # of parts done via Parallel()
        igpar = []
        igser = []
        ...
        print(
            "fof6d : Doing %d groups with npart<%d on single core [t=%.2f s]"
            % (len(igser), npart_par, time.time() - t0)
        )
        for igrp in igser:
            results[igrp] = fof6d(
                igrp,
                groups,
                pos.T[groups[igrp][0] : groups[igrp][1]],
                vel.T[groups[igrp][0] : groups[igrp][1]],
                kerneltab,
                t0,
                Lbox,
                fof_LL,
        ...
        print(
            "\nfof6d : Doing %d groups with npart>%d on %d cores (progressbar approximate) [t=%.2f s]"
            % (len(igpar), npart_par, nproc, time.time() - t0)
        )
        if len(igpar) > 0:
            import multiprocessing
            from joblib import Parallel, delayed

            results_par = Parallel(n_jobs=nproc)(
                delayed(fof6d)(
                    igrp,
                    groups,
                    pos.T[groups[igrp][0] : groups[igrp][1]],
        ...
        for i in range(len(igpar)):
            results[igpar[i]] = results_par[i]
    else:
        print(
            "fof6d : Doing %d groups on single core [t=%.2f s]"
            % (len(groups), time.time() - t0)
        )
        for igrp in range(len(groups)):
            results[igrp] = fof6d(
    ...
    for igrp in range(len(groups)):
        ...
        galindex[istart:iend] = np.where(
            result[1] >= 0, result[1] + nfof, -1
        )  # for particles in groups, increment group ID with counter (nfof)
        nfof += result[0]

    gindex, sindex, bindex = reset_order(
        galindex, pindex, ngastot, nstartot, nbhtot, snap
    )
        ...
        for ifof in range(nfof):
            ...
            if ifof < 2:
                print(
                    ...
                    np.mean(sgrp[1]),
                    np.mean(sgrp[2]),
                    max(sgrp[0]) - min(sgrp[0]),
                    max(sgrp[1]) - min(sgrp[1]),
                    max(sgrp[2]) - min(sgrp[2]),
                )
                print(len(sgrp.T), sgrp.T)

    print(
        "\nfof6d: Found %d galaxies, with %d gas+%d star+%d BH = %d particles [t=%.2f s]"
        % (
            nfof,
            len(gindex[gindex >= 0]),
            len(sindex[sindex >= 0]),
...
def run_fof_6d(snapfile, mingrp, LL_factor, vel_LL, nproc):
    ...
    if nproc != 1:  # if multi-core, set up Parallel processing
        ...
        if nproc < 0:
            print(
                "fof6d : Using %d cores (all but %d)"
                % (num_cores + nproc + 1, -nproc - 1)
            )
        if nproc > 1:
            print("progen : Using %d of %d cores" % (nproc, num_cores))
        else:
            print("progen : Using single core")
        if nproc > 8:
            print(
                "fof6d : FYI you are using nproc=%d. nproc>8 tends to give minimal or negative benefit."
                % nproc
            )

    # find friends of friends groups
    gas_index, star_index, bh_index, t0 = fofrad(
        snapfile, nproc, mingrp, LL_factor, vel_LL
    )  # returns galaxy indices for *all* gas, stars, bh
